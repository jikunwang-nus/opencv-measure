...
def pre_handle(img) -> any:
    # 高斯模糊
    img_gas = cv.GaussianBlur(img_rgb, (13, 13), 0)
    # 边缘检测
...
def print_err_line_point(cur_row):
    img_err = cv.imread(imgPath)
...
# def write_final_bd_png(merge_stats, original_point, skip):
...
#     cv.imwrite('final-bd.png', img_rgb)
#     cv.imwrite('original-after-bd-red.png', img_rgb)


def write_png(merge_stats, original_point, skip, mode):
    valid_count = 0
    for a in merge_stats:
        if mode == 'original':
            x, y, w, h, s = a
...
if __name__ == '__main__':
    ...
    # 打印图像标定
    write_png(merge_stats, original_point, skip, 'handled')
    # 计算距离
